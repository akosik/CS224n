...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        for center_index,label in enumerate(labels):
            features = []
            for window_index in xrange(center_index - window_size, center_index + window_size + 1):
                if window_index < 0:
                    features += start
                elif window_index >= sentence_length:
                    features += end
                else:
                    features += [ feature for feature in sentence[window_index] ]
...
class WindowModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        W = tf.Variable( initializer(shape=[self.config.n_window_features*self.config.embed_size,self.config.hidden_size]) )
        ...
        pred = tf.matmul(tf.nn.dropout(tf.nn.relu(tf.matmul(x,W) + b1), keep_prob=dropout_rate), U) + b2
        ### END YOUR CODE
        return pred
